...
public class SessionPool<I, R> extends GenericObjectPool<Session<I, R>> {
	 ...
	 * 		The top-level <code>DiagnosisEngineConfiguration</code>. Must not be null!
	...
	public SessionPool(DiagnosisEngineConfiguration<I, R> engineConfiguration, GenericObjectPool.Config poolConfiguration) {
		super(new SessionFactory<>(checkNotNull(engineConfiguration)));
		setConfig(checkNotNull(poolConfiguration, "GenericObjectPool.Config must not be null!"));
	}
