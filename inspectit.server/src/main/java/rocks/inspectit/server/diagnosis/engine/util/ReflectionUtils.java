...
public class ReflectionUtils {
    ...
    /**
     * Represents a callback while searching for annotations on a type. The <code>Visitor</code> is used to lookup a certain annotation on a type, utilize it, and return something.
     *
     * @param <A> The annotation to by searched.
     * @param <T> The type where containing the annotation.
    ...
    public interface Visitor<A extends Annotation, T, R> {
         ...
         * @param type       The type which owns the annotation.
         * @return Any object.
         */
     ...
     * @param annotationClass The annotation to be looked up.
    ...
    /**
     * Visits all fields of a class which are annotated with a certain annotation. The method utilizes a {@link Visitor} to create the results.
     * The following example shows how all fields, annotated with SessionVariable, are processed and wrapped in SessionVariableInjection objects.
     * <p>
     * <pre>
     *  {@code
     *  List<SessionVariableInjection> injections = ReflectionUtils
     *      .visitFieldsAnnotatedWith(SessionVariable.class, clazz, new Visitor<SessionVariable, Field, SessionVariableInjection>() {
     *      @Override
     ...
     * </pre>
     *
     * @param annotation The ype of the annotation.
     * @param clazz      The class to be checked.
     * @param visitor    The {@link Visitor} to process the annotated field.
     * @param <A>        The Annotation type
     * @param <R>        The result type of the {@link Visitor}
     * @return List withe elements of type <code>R</code> produced by the {@link Visitor}.
     */
    ...
    /**
     * Visits all methods of a class which are annotated with a certain annotation. The method utilizes a {@link Visitor} to create the results.
     * The following example shows how all methods, annotated with Action, are processed and wrapped in ActionMethod objects.
     * <p>
     * <pre>
     *  {@code
     *  List<ActionMethod> actionMethods = ReflectionUtils.
     *      visitMethodsAnnotatedWith(Action.class, clazz, new Visitor<Action, Method, ActionMethod>() {
     *      @Override
